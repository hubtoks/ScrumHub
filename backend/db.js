...
// MySQL 列名是 snake_case，前端期望 camelCase，此处统一转换
...
async function query(sql, params = []) {
  ...
  // DDL 语句（CREATE/ALTER/DROP）返回 OkPacket，不是数组，不用转换
...
async function initDatabase() {
  ...
  await query(createUserStories)
  await query(createIterations)
  await query(createRetrospects)
  console.log('[DB] 数据表已就绪（user_stories, iterations, retrospects）')
}

// 生成唯一ID（保持和原来一样的格式，用于兼容已有数据）
function generateId() {
